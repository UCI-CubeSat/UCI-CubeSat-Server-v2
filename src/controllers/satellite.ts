...
satelliteController.use(authMiddleware);
...
satelliteController.post(
  "/by_time_range",
  ensureResponse(
    async (req, res: Response<PostRangeResBody | GenericErrorResponse>) => {
      try {
        const body = customErrorIfSafeParseError(
          PostByRangeReqBodyValidator.safeParse(req.body),
          RequestBodyError
        );
...
satelliteController.post(
  "/by_offset",
  ensureResponse(
    async (req, res: Response<PostOffsetResBody | GenericErrorResponse>) => {
      try {
        const body = customErrorIfSafeParseError(
          PostOffsetReqBodyValidator.safeParse(req.body),
          RequestBodyError
        );
        const databaseLogs = await getLogsByOffset(
          (body.pageNo - 1) * body.count,
          body.count
        );
        res.status(200).send({
          logs: databaseLogs,
          numLogsAfter: await countLogsAroundCursor(
            databaseLogs[databaseLogs.length - 1].timestamp,
            "forward"
          ),
          numLogsBefore: await countLogsAroundCursor(
            databaseLogs[0].timestamp,
            "backward"
          ),
        });
      } catch (e) {
...
satelliteController.post(
  "/by_cursor",
  ensureResponse(
    async (req, res: Response<PostPaginatedResBody | GenericErrorResponse>) => {
      try {
        const body = customErrorIfSafeParseError(
          PostCursorReqBodyValidator.safeParse(req.body),
          RequestBodyError
        );
